...
public class MainActivity extends FragmentActivity implements OnMapReadyCallback
{
    ...
    private List<Marker> sharedLocationMarkers = new ArrayList<>();
    ...
    private void setupButtons()
    {
        ...
        startStopButton.setOnClickListener(v ->
        {
            ...
            if (!isSharingLocation) {
                ...
                startStopButton.setText("위치 숨기기");
            }else {
                // 마커 리스트에 저장된 것만 제거
                for (Marker marker : sharedLocationMarkers) {
                    marker.remove();
                }
                sharedLocationMarkers.clear();
                isSharingLocation = false;
                startStopButton.setText("위치 공유");
            }
        });
    }
    ...
    private void addGroupMarkers() {
        ...
        List<LatLng> Locations = Arrays.asList(
                new LatLng(37.5665, 126.9780), // 서울
                new LatLng(37.3943, 127.1115), // 경기 성남
                new LatLng(37.2635, 127.0286)  // 경기 수원
        );

        String[] names = {"지수", "민준", "예린"};
        String[] statuses = {"도착", "이동중", "도착"};

        for (int i = 0; i < Locations.size(); i++) {
            LatLng loc = Locations.get(i);
            String title = names[i];
            String status = "상태: " + statuses[i];
            ...
            sharedLocationMarkers.add(marker); // 리스트에 저장
        }
